...
import { Calendar, Users, FileText, Pencil, CheckCircle, XCircle, Send, Clock } from "lucide-react";
...
export default function QuoteCenterCard({ quote, group, profile, optionPricing, canDecide, onEdit, onApprove, onReject, onDeleted, onUpdated, onStatusChange }) {
  ...
  return <div className="bg-card border border-border rounded-xl p-4 space-y-3" dir="rtl">
    ...
    <div className="flex flex-wrap gap-2">{group && <RoleGate roles={["SUPER_ADMIN", "ADMIN", "QUOTES_MANAGER"]}><Button asChild size="sm" variant="outline"><Link to={`/groups/${group.id}`}><FileText className="w-3.5 h-3.5" />פתח קבוצה</Link></Button></RoleGate>}<QuotePdfButton quote={quote} group={group} /><QuoteSyncButton quote={quote} group={group} profile={profile} onSynced={onUpdated} /><RoleGate permission="EDIT_QUOTE"><Button size="sm" variant="ghost" onClick={onEdit}><Pencil className="w-3.5 h-3.5" />עריכה</Button></RoleGate><QuoteDeleteButton quote={quote} onDeleted={onDeleted} />{canDecide && quote.status === "DRAFT" && <Button size="sm" variant="outline" onClick={() => onStatusChange("SENT")}><Send className="w-3.5 h-3.5" />סמן כנשלח</Button>}{canDecide && quote.status === "SENT" && <Button size="sm" variant="outline" onClick={() => onStatusChange("EXPIRED")}><Clock className="w-3.5 h-3.5" />פג תוקף</Button>}{canDecide && ["DRAFT","SENT"].includes(quote.status) && <><Button size="sm" onClick={() => quote.multi_option_enabled ? setOptionDialogOpen(true) : onApprove("A")}><CheckCircle className="w-3.5 h-3.5" />אשר הצעה</Button><Button size="sm" variant="destructive" onClick={onReject}><XCircle className="w-3.5 h-3.5" />דחייה</Button></>}</div>
