...
export default async function(req) {
  ...
  try {
    ...
    const plan = body.action === 'READD' ? planScopedReAdd(context, body) : planScopedRelease(context, body);
    ...
    return Response.json({ success: true, action: body.action === 'READD' ? 'READD' : 'RELEASE', affected_period_count: plan.affectedPeriods.length, warnings: plan.warnings, historical_rows_unchanged: true });
  } catch (error) {
