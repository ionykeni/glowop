...
import { planScopedAdd, planScopedReAdd, planScopedRelease } from '../../shared/sleepingPeriodScope.js';
...
export default async function(req) {
  ...
  try {
    ...
    return Response.json({ success: true, action: ['ADD', 'READD'].includes(body.action) ? body.action : 'RELEASE', affected_period_count: plan.affectedPeriods.length, warnings: plan.warnings, historical_rows_unchanged: true });
  } catch (error) {
