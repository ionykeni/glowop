...
export default async function(req) {
  ...
  try {
    ...
    if(!plan.allowed) return Response.json({success:false,error:'לא ניתן לשמור: קיימת התנגשות בתכנית הלינה',exact_tent_conflicts:plan.exact_tent_conflicts,neighborhood_conflicts:plan.neighborhood_conflicts});
    ...
    return Response.json({success:true,already_committed:!fresh.length&&!plan.updates.length,pax_edit:plan.updates.length>0,sleeping_rows_created:created.length,sleeping_rows_updated:plan.updates.length,sleeping_allocation_ids:created.map(r=>r.id),historical_rows_unchanged:true});
  } catch(error) {
    const recovery=writes?await writes.rollback():null;
    return Response.json({success:false,error:recovery&&!recovery.restored?'השמירה לא הושלמה ונדרשת בדיקת השיבוץ לפני ניסיון נוסף':error.message,recovery});
  }
}